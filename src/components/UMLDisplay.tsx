...
const highlightUMLSyntax = (code: string): string => {
  return code
    ...
    .map(line => {
      ...
      line = line.replace(/(^|\s)(if|else|endif|alt|end|then|elseif)(\s|$)/g, 
        '$1<span class="text-purple-400">$2</span>$3');
      
      // PlantUML-Befehle
      line = line.replace(/(^|\s)(@startuml|@enduml|title|start|stop)(\s|$)/g,
        '$1<span class="text-blue-400">$2</span>$3');
      
      // Akteure und Teilnehmer
      line = line.replace(/(^|\s)(actor|participant|boundary|control|entity|database)(\s|$)/g,
  ...
  return (
    ...
    >
      ...
      <Card className={`uml-display-container ${
        type === "human" ? "uml-display-human border-uml-human/30" : "uml-display-llm border-uml-llm/30"
        ...
        {isEditing ? (
          ...
          <div className="w-full h-full bg-[#f8f9fa] overflow-y-auto">
            <div className="w-full min-h-full p-4" style={{
